...
class MagazineDocumentWriter
  def initialize(output, supplier, commodity, quanity, date, current_number, humidity, contamination, type, vehicle_number, symbol_of_evidence, prof_of_delivery, no, destination)
    @output = output
    @supplier = supplier
    @commodity = commodity
    @quanity = quanity
  ...
  def write
    ...
    fields.push Field.new(@date, 683, 80, 77, 22)
